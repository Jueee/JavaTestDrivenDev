...
public class Books {
    ...
    public Books filterByState(String state) {
        ...
        return new Books(new ConcurrentLinkedQueue<>(books.stream().filter(x -> x.anyState().contains(expectedState)).collect(toList())));
    }
    ...
    public Books filterOutCensored() {
        return new Books(new ConcurrentLinkedQueue<>(books.stream().filter(x -> x.getStatus() != States.CENSORED.getValue()).collect(toList())));
    }
}
