...
public class BooksEndpoint {
    ...
    /**
     * 	一、
     * 	将创建依赖的代码放在BookRepository 类的一个受保护的方法中：
     *	@return
     */
    ...
    public Response rentBook(@PathParam("id") String id, @PathParam("user") String userId) {
        ...
        if (null == id || null == userId || matchingBooks.isEmpty() || users.findById(userId).isEmpty() || matchingBooks.first().getStatus() != States.AVAILABLE.getValue()) {
    ...
    public Response returnBook(@PathParam("id") String id) {
        ...
        if (null == id || matchingBooks.isEmpty() || matchingBooks.first().getStatus() != States.RENTED.getValue()) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Could not process this request").build();
        }
    ...
    public Response censorBook(@PathParam("id") String id) {
        final Books matchingBooks = books.list().filterById(id);
        if (null == id || matchingBooks.isEmpty() ||matchingBooks.first().getStatus() == States.CENSORED.getValue()) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Could not process this request").build();
        }
    ...
    public Response initialPreparation(@PathParam("id") String id) {
        final Books matchingBooks = books.list().filterById(id);
        if (null == id || matchingBooks.isEmpty() || matchingBooks.first().getStatus() != States.BOUGHT.getValue()) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Could not process this request").build();
        }
    ...
    public Response addNewBook(
        ...
        books.add(new Book(title, author, States.BOUGHT.getValue()));
